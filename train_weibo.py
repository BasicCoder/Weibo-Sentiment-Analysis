...
class MyPro(DataProcessor):
    ...
    '''
    def get_train_examples(self, data_dir, filename="train.json"):
        return self._create_examples(
            self._read_json(os.path.join(data_dir, filename)), 'train')

    def get_dev_examples(self, data_dir, filename="val.json"):
        return self._create_examples(
            self._read_json(os.path.join(data_dir, filename)), 'dev')

    def get_test_examples(self, data_dir, filename="test.json"):
        return self._create_examples(
...
def test(model, processor, args, label_list, tokenizer, device):
    ...
    Returns:
        f1: F1值
    '''
    test_examples = processor.get_test_examples(args.data_dir, filename=args.test_filename)
    ...
    for input_ids, input_mask, segment_ids, label_ids in test_dataloader:
        ...
        with torch.no_grad():
            ...
            score_pred = softmax_logits[:,1]
            predict = np.hstack((predict, pred.cpu().numpy()))
            score = np.hstack((score, score_pred.cpu().numpy()))
            gt = np.hstack((gt, label_ids.cpu().numpy()))
    ...
    base_filename = os.path.splitext(args.test_filename)[0]
...
def main():
    ...
    parser.add_argument("--train_filename", default="train.json", type=str, help="The name of train file")
    parser.add_argument("--val_filename", default="val.json", type=str, help="The name of validate file")
    parser.add_argument("--test_filename", default="test.json", type=str, help="The name of test file")
    ...
    # Prepare model
    logger.info(PYTORCH_PRETRAINED_BERT_CACHE / 'distributed_{}'.format(args.local_rank))
    ...
    if args.local_rank != -1:
        logger.info("Use DistributedDataParallel")
